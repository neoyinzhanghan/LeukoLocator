...
save_path = (
    "/media/ssd2/dh_labelled_data/DeepHeme1/MSK_repo_mixed/mskcc_bma_abnormal.csv"
)
    ...
    for subfolder in os.listdir(cartridge_path):
        if len(subfolder) < 5:
            for f in os.listdir(os.path.join(cartridge_path, subfolder)):
                # check if the file is a .png file and the file name stem is not numeric string
                if f.endswith(".png") and not f.split(".")[0].isnumeric():
